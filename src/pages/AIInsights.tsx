...
export default function AIInsights() {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      if (e.message !== "rate_limited" && e.message !== "payment_required") {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] max-h-[calc(100vh-4rem)]">
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-border/40">
        ...
        <div>
          <h1 className="text-lg font-bold text-foreground">AI Insight Assistant</h1>
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-6">
            ...
            <div className="text-center max-w-md">
              ...
              <p className="text-sm text-muted-foreground">
                Ask me anything about your programs, beneficiaries, finances, or
                staff performance.
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-lg">
              {SUGGESTED_QUESTIONS.map((q) => (
                <button
                  key={q}
                  onClick={() => send(q)}
                  className="text-left px-4 py-3 rounded-xl border border-border/60 bg-card hover:bg-accent/10 hover:border-primary/30 transition-all text-sm text-foreground"
                >
                  {q}
                </button>
              ))}
          ...
          messages.map((msg, i) => (
            ...
            >
              ...
              >
                {msg.role === "assistant" ? (
                  <div className="prose prose-sm dark:prose-invert max-w-none text-foreground [&_table]:text-xs [&_th]:px-2 [&_td]:px-2 [&_th]:py-1 [&_td]:py-1 [&_table]:border-border [&_th]:border-border [&_td]:border-border">
      ...
      <div className="border-t border-border/40 px-4 py-3">
        <div className="flex items-end gap-2 max-w-3xl mx-auto">
          <Textarea
            ...
            className="min-h-[44px] max-h-[120px] resize-none rounded-xl border-border/60 bg-card focus-visible:ring-primary/30"
            rows={1}
            disabled={isLoading}
          />
          <Button
        ...
        <p className="text-[10px] text-muted-foreground text-center mt-2">
          Powered by Ufanisi AI · Responses are based on your organization's live data
        </p>
      </div>
    </div>
  );
