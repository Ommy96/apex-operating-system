...
import { OtherReportForm } from "@/components/OtherReportForm";
...
export default function OtherReports() {
  ...
  const { data: reportStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const uniqueMonths = Array.from(new Set(activityReports?.map(r => {
        const date = new Date(r.reporting_date);
        return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
      }) || [])).sort().reverse();
      
      return { totalReports, thisMonth, programBreakdown, uniqueStaff, staffList, uniqueMonths };
    },
  ...
  const handleDownload = () => {
    if (!filteredReports || filteredReports.length === 0) {
      toast({
        title: "No Data",
        description: "No other reports to download",
        variant: "destructive",
      });
      return;
    }
    ...
    downloadExcel(formattedData, 'Other_Reports', 'Other Reports');
    
    toast({
      title: "Success",
      description: "Other reports downloaded successfully",
    });
  };
  ...
  const handleDelete = async (reportId: string, report: any) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Other report deleted successfully",
      });
      ...
      {filteredReports?.length === 0 && (
        <div className="text-center py-12">
          <p className="text-muted-foreground">
            {isStaff ? "You have not submitted any other reports yet." : "No other reports found."}
          </p>
        </div>
      )}
