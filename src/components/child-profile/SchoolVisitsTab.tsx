...
import { GraduationCap, Plus, Calendar, User, MapPin, Eye, Edit, Trash2, FileText, AlertCircle, School } from 'lucide-react';
import { supabase } from '@/integrations/supabase/client';
import { toast } from '@/hooks/use-toast';
import { SchoolVisitReportForm } from '@/components/SchoolVisitReportForm';

interface SchoolVisitsTabProps {
  childId: string;
  childSchool?: string | null;
  isAdmin: boolean;
}

export function SchoolVisitsTab({ childId, childSchool, isAdmin }: SchoolVisitsTabProps) {
  const queryClient = useQueryClient();
  const [isCreateDialogOpen, setIsCreateDialogOpen] = useState(false);
  const [editingReport, setEditingReport] = useState<any>(null);
  const [viewingReport, setViewingReport] = useState<any>(null);

  // Fetch school visit reports for this child
  const { data: schoolVisits = [], isLoading } = useQuery({
    queryKey: ['child-school-visits', childId],
    queryFn: async () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['child-school-visits', childId] });
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h3 className="text-xl font-bold">School Visit Reports</h3>
          <p className="text-muted-foreground text-sm">{schoolVisits.length} visit{schoolVisits.length !== 1 ? 's' : ''} recorded</p>
        </div>
        {isAdmin && (
          <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Record School Visit</DialogTitle>
              </DialogHeader>
              <SchoolVisitReportForm
                onSuccess={handleFormSuccess}
                onCancel={() => setIsCreateDialogOpen(false)}
                preselectedChildId={childId}
                preselectedSchool={childSchool}
              />
            </DialogContent>
          </Dialog>
        )}
      ...
      {schoolVisits.length === 0 ? (
        <Card className="border-dashed border-2 bg-secondary/20">
          <CardContent className="text-center py-12">
            ...
            <p className="text-muted-foreground font-medium">No school visits recorded</p>
            <p className="text-sm text-muted-foreground/70">Add a school visit report to track visits</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="p-3 rounded-lg bg-secondary/50">
                    ...
                    <p className="font-medium">{viewingReport.school}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-secondary/50">
      ...
      <Dialog open={!!editingReport} onOpenChange={() => setEditingReport(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Edit School Visit Report</DialogTitle>
          </DialogHeader>
          {editingReport && (
            <SchoolVisitReportForm
