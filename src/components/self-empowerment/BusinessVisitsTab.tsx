...
import { Plus, Building2, Calendar, User, Eye, Edit, Trash2, MapPin, AlertCircle, CheckCircle2, Clock, Download } from "lucide-react";
import { format } from "date-fns";
import { useToast } from "@/hooks/use-toast";
import { useAuth } from "@/hooks/useAuth";
import { useOrganization } from "@/hooks/useOrganization";
import { BusinessVisitForm } from "./BusinessVisitForm";
import * as XLSX from 'xlsx';

interface BusinessVisitsTabProps {
  applicantId?: string;
  applicantName?: string;
}

export function BusinessVisitsTab({ applicantId, applicantName }: BusinessVisitsTabProps) {
  ...
  // Fetch business visit reports for this applicant
  const { data: businessVisits = [], refetch } = useQuery({
    queryKey: ['business-visits', applicantId, currentOrganization?.organization_id],
    queryFn: async () => {
      ...
      if (applicantId) {
        query = query.eq('business_id', applicantId);
      }
  ...
  const handleDownload = () => {
    if (!businessVisits || businessVisits.length === 0) {
      toast({ title: "No data to download", variant: "destructive" });
      return;
    }

    const formattedData = businessVisits.map((visit: any) => ({
      'Staff': visit.staff,
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Business Visits');
    XLSX.writeFile(wb, `business_visits_${applicantName || 'all'}_${new Date().toISOString().split('T')[0]}.xlsx`);
    toast({ title: "Business visits exported successfully" });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {applicantName ? `Visit history for ${applicantName}` : 'All business visit reports'}
          </p>
        </div>
        <div className="flex gap-2">
          {(isAdmin || isManagement) && businessVisits.length > 0 && (
      ...
      {businessVisits.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">No Business Visits Yet</h3>
            <p className="text-muted-foreground mb-4">
              Start documenting business visits to track progress and support.
            </p>
            {isAdmin && (
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingVisit ? 'Edit Business Visit' : 'Record Business Visit'}
            </DialogTitle>
          </DialogHeader>
          <BusinessVisitForm
