...
import { DonorProgressReport } from "@/components/reports/DonorProgressReport";
    ...
    <div className="space-y-4">
      {[1, 2, 3].map(i => (
        <Card key={i}><CardContent className="p-4 space-y-3">
          ...
          <div className="flex gap-2"><Skeleton className="h-3 w-1/3" /><Skeleton className="h-3 w-1/4" /></div>
        </CardContent></Card>
      ))}
    </div>
  );
        ...
        <Tabs defaultValue="overview" className="w-full">
          <div className="overflow-x-auto no-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
             <TabsList className="inline-flex w-max gap-1 bg-muted/50 p-1 rounded-xl">
              ...
              <TabsTrigger value="progress-report" className="text-xs rounded-lg">Progress Report</TabsTrigger>
