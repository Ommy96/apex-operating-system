...
// SupportTicket type removed - table dropped
...
// PlatformAnnouncement type removed - table dropped
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const orgsWithCounts = await Promise.all(
        (data || []).map(async (org) => {
          const [membersRes, beneficiariesRes] = await Promise.all([
...
// Support tickets removed - table dropped
...
// Platform announcements removed - table dropped
