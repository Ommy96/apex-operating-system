...
describe('isSuperAdmin', () => {
  it('returns true for the configured super admin email when env is set', () => {
    if (SUPER_ADMIN_EMAIL) {
      expect(isSuperAdmin(SUPER_ADMIN_EMAIL)).toBe(true);
    } else {
      // If env var not set, all checks return false
      expect(isSuperAdmin('any@email.com')).toBe(false);
    }
  });
